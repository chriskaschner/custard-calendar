...
Pure renderer: receives flavor data via config params, displays with color-coded icons.
Data is fetched by the Python orchestrator and passed via pixlet render arguments.
"""
...
def create_ice_cream_icon(profile):
    """Create an ice cream cone icon with toppings and a cone shape (single-day view)."""
    color = BASE_COLORS[profile["base"]]

    # Derive topping color from profile
    toppings = profile.get("toppings", [])
    if len(toppings) > 0:
        topping_color = TOPPING_COLORS[toppings[0]]
    elif profile.get("ribbon"):
        topping_color = RIBBON_COLORS[profile["ribbon"]]
    else:
...
def create_mini_cone(profile):
    """Create a mini ice cream cone (9x11) per cone_spec.md profile-driven rendering.
    ...
    # Topping pixels in fixed slots
    # T1: interior(2,2) -> pad(2,1)
    # T2: interior(6,2) -> pad(6,1)
    # T3: interior(3,4) -> pad(3,3)
    # T4: interior(5,3) -> pad(5,2) -- skip if ribbon present (R3 collision)
    ...
    # Ribbon pixels (rendered after toppings so ribbon wins at R3/T4 overlap)
    # R1: interior(3,1) -> pad(3,0)
    # R2: interior(4,2) -> pad(4,1)
    # R3: interior(5,3) -> pad(5,2)
    if has_ribbon:
    ...
    return render.Stack(
        children = [
            # Size anchor (9x11, transparent/black background)
...
def format_flavor_for_display(name, max_chars = 5):
    ...
    base_nouns = [
        "Avlnc", "Volc", "Chees", "Flury", "Cake", "Crumb",
        "Bliss", "Ovld", "Dough", "Pecan", "Cashw", "Fudge", "Crml",
        "Choc", "Expl", "Delit", "Dream", "Swirl", "Crunc",
        "Cook", "Brown", "Twist", "Turtl", "Mint", "Toff", "Decad",
        "Straw", "PBcup", "Dove"
    ]
...
def main(config):
    """Main entry point - reads flavor data from config params.

    Flavor data is passed as flattened key=value pairs by the Python orchestrator:
        flavor_0, flavor_date_0, flavor_1, flavor_date_1, ...
    """
    ...
    flavors = []
    for i in range(10):
        name = config.get("flavor_{}".format(i))
        date = config.get("flavor_date_{}".format(i))
        if name:
            flavors.append({
                "name": name,
    ...
    # Fallback if no flavors passed
    if len(flavors) == 0:
        return render.Root(
            child = render.Box(
                child = render.Text("No flavors", font = "tom-thumb"),
            ),
        )
