...
- [x] **T01: Align rarityLabel() thresholds and add consistency test** `est:20m`
...
- **Failure signal:** A flavor at 130 days gap appearing as "Ultra Rare" on OG cards but "Rare" on the homepage is the canonical observable symptom of threshold drift. The consistency test explicitly pins this case (`rarityLabel(130)` → `'Rare'`).
- **Inspection surface:** `rarityLabel` is now exported from `social-card.js`, so any test or REPL can call it directly with a days value. The function's JSDoc comment lists the canonical thresholds (>150 Ultra Rare, >90 Rare).
- **Drift detection:** `worker/test/rarity-threshold-consistency.test.js` runs in every `npm test` invocation. A threshold change in any of the three files without a corresponding change in the others will cause the test suite to fail immediately.
- **Grep surfaces:** `rg ">120|>60" worker/src/social-card.js` (zero matches = correct) and `rg "> 150|> 90" worker/src/social-card.js` (two matches = correct) are the fastest manual inspections.
- **Redaction:** No secrets or PII involved; rarity labels are purely computed from public flavor gap data.
