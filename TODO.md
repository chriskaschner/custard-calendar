...
- [ ] **Intra-WI metro leaderboards** -- break WI results down by metro area (Madison, MKE, other). Requires a metro field in store-index or a zip-to-metro mapping. Surface as "Madison's top flavor vs MKE's top flavor."
- [ ] **Geo-aware leaderboard quiz questions** -- trivia questions about "most popular flavor in your state" should be filtered to the user's detected state (from geoip). A WI user should never see a question about MI flavor rankings.
...
- [x] **CLAUDE.md doc fixes** -- updated Worker test count (343 -> 567, 21 -> 32 suites), page count (5 -> 9), added analytics_report.py command. (2026-02-25)
