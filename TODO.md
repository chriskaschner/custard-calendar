...
- [ ] **Refresh the forecast pipeline -- by repointing at D1, not by refreshing the backfill.** `/api/v1/forecast/{slug}` serves predictions generated 2026-02-23 covering only through 2026-03-02, so `buildTimeline()`'s `predicted` gap-fill contributes nothing and nothing warns that a 5-month-old forecast is being served. D1 `forecasts` holds 517 rows, all stamped `2026-02-23T11:22:23` -- one batch, never refreshed.
    - **The training corpus is the actual problem, and the fix is not what it looks like.** `analytics/data_loader.py:15` hardcodes `data/backfill/flavors.sqlite`: 59,987 rows, 522 stores, ending 2026-03-31. D1 `snapshots` already holds **210,605 rows across 1,002 stores spanning 2015-08-02 to 2026-09-30**, and the daily harvest keeps it current. D1 is strictly better on every axis, so repoint the loader at it (or export D1 to sqlite as a drop-in) rather than rebuilding the backfill.
    - Then re-run `analytics.batch_forecast` and `scripts/upload_forecasts.py`. That upload path is `wrangler d1 execute`, so it is independent of KV.
    - Add a staleness guard so an expired forecast is never served silently as if fresh.
