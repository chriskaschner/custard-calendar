...
- [ ] **Data quality testing strategy review** -- the mt-horeb D1 sparseness issue (10 rows vs 53 in local SQLite) went undetected because there was no automated gate comparing D1 depth against local backfill. Groundwork is now in place: `scripts/check_backfill_coverage.py` (exits nonzero if D1 < 90% of local for priority stores) + 41 unit tests. Remaining work: (1) wire `check_backfill_coverage.py` into a scheduled CI job or post-deploy hook so it runs automatically after each D1 upload; (2) extend the pattern to other data quality axes -- forecast staleness, snapshot freshness per store, metrics seed age; (3) research: evaluate whether a lightweight "data contract" layer (e.g. Great Expectations, Pandera, or custom assertions) would reduce the category of silent data drift bugs across the pipeline; (4) add a flavor-stats endpoint test that asserts appearances for a known flavor at a known store exceed a minimum threshold (requires mocked D1 with realistic fixture data).
...
- [ ] **Upload full backfill corpus to D1** -- `scripts/upload_backfill.py` is ready. Run `--stores mt-horeb,verona,madison-todd-drive` first, then `--all-wi`, then `--all`. Verify D1 row counts match local SQLite counts for priority stores post-upload.
...
- [ ] **Fix cross-store avg_gap calculation in flavor-hierarchy** -- state/national scope avg_gap is computed by deduplicating dates across all stores in the scope, then averaging consecutive gaps. This collapses same-day appearances from multiple stores into one date, producing artificially small gaps (e.g. avg_gap=1d for state scope when different MN stores have Mint Cookie on adjacent days). Fix: either (a) average per-store avg_gap values across slugs in the scope, or (b) compute avg_gap as total_span / appearances where appearances counts store-days, not deduped calendar days. Store-scope avg_gap is correct and unaffected.
- [ ] **Geographic EDA endpoint** -- "what's hiding in the data at each scope". A dedicated analytics view surfacing aggregate patterns per metro/state/national: top flavors appearing exclusively at one metro, flavors with widest regional variance in cadence, stores that are statistical outliers vs their metro peer group. Separate from the per-query hierarchy endpoint -- this is a batch/exploratory report, not a live lookup.
