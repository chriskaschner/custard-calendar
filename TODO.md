...
- [ ] **Canonical render spec** -- palette + geometry + toppings with adapters per surface.
  - [ ] **Hero cone renderer reevaluation** -- `renderConeHeroSVG` + audit `heroCone()` client mirror are implemented but produce unacceptable results (blocking highlight squares, ribbon too narrow, shading reads as color artifacts not depth). Needs full redesign against `docs/cone-profile-spec.md` and reference pixel art before wiring into OG cards. Deprioritized below all Now/Next tasks; promote above Someday/Maybe.
