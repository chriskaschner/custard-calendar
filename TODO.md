...
- [x] **P0: make a root-level `wrangler deploy` impossible** -- done 2026-08-01. `npm run deploy` now runs `worker/scripts/predeploy-check.mjs` and pins `--config wrangler.toml`, so cwd cannot decide which config wins. The check refuses to run outside `worker/`, refuses a config missing `main` or `name`, and refuses while a stray `wrangler.jsonc`/`.json`/`.toml` sits at the repo root (printing the `rm` command, since the file is gitignored and invisible to `git status`). 11 tests in `worker/test/predeploy-check.test.js`, including a reproduction of the exact config that caused the outage. Deploy discipline documented in CLAUDE.md.
    - **Residual risk:** the guard only covers `npm run deploy`. A bare `npx wrangler deploy` from the wrong directory is still dangerous and cannot be blocked from inside the repo. The verification tell is unchanged: a correct deploy lists the three `schedule:` cron triggers, a no-op asset deploy omits them.
    - Original context: running `wrangler deploy` from the repo root scaffolds `/wrangler.jsonc` with `name = "custard-calendar"` -- the *same name as production* -- plus `assets: { directory: "public" }` and **no `main`**. With no `main`, wrangler builds a no-op asset server and the deploy replaces the real Worker with 4 stale Hugo XML files. Took production down on both domains on 2026-08-01, 404 on everything including `/v1/calendar.ics`. The pre-existing `.gitignore` entry suggests this had happened before.
...
- [x] **Refresh the forecast pipeline -- by repointing at D1, not by refreshing the backfill.** -- done 2026-08-02 via PR #17 (`2ef105a`). `analytics/d1_source.py` added; `data_loader.py` now reads D1 instead of the frozen sqlite. `worker/src/forecast.js` refuses to serve an expired forecast rather than passing a stale one off as fresh. Batch re-run confirmed live the same day: `/api/v1/forecast/mt-horeb` returns `generated_at: 2026-08-02T08:07:30` with `history_depth: 1625`, covering 2026-08-03 forward (previously stamped 2026-02-23 and covering only through 2026-03-02).
    - **Not yet deployed.** The staleness guard is merged but the Worker has not been redeployed, so the guard is not live. It is currently inert either way -- the served forecast is fresh -- but it will not protect anything until `cd worker && npm run deploy` runs. Verify the deploy output lists the three `schedule:` cron triggers.
    - Original context: `analytics/data_loader.py:15` hardcoded `data/backfill/flavors.sqlite` (59,987 rows, 522 stores, ending 2026-03-31) while D1 `snapshots` already held 210,605 rows across 1,002 stores spanning 2015-08-02 to 2026-09-30.
...
- [ ] **Check the 2026-08-05 premiere day outcome.** 2026-08-05 is the first Wednesday of August and the first premiere day to fall *after* the placeholder fix shipped, so it is the first real test of the whole path. As of 2026-08-02 the placeholder is rendering correctly -- `/api/v1/flavors?slug=mt-horeb` returns `2026-08-05 | Not yet announced`, with every surrounding day carrying a real flavor.
    - What to verify on or after 2026-08-05: the placeholder is *replaced* by the real flavor once Culver's publishes it, rather than sticking. Sticking would mean the override outlives its usefulness and starts masking real upstream data -- the opposite of the failure it was built to fix.
    - Also confirm no placeholder leaked into KV, D1, or the flavor catalog. Those writes are supposed to be blocked at the serving boundary.
...
- [ ] **P0 BLOCKER: route `/sitemap.xml`, `/robots.txt`, and `/store/*` to the Worker.** Phase 37 is deployed and correct on `custard-calendar.chris-kaschner.workers.dev`, but on `custard.chriskaschner.com` all three fall through to GitHub Pages and 404. The sitemap the Worker generates points at `https://custard.chriskaschner.com/store/...` URLs that do not resolve, so Google would index nothing. **Phase 37 is functionally unshipped until this is fixed.**
    - **Re-verified 2026-08-02, still broken.** `https://custard.chriskaschner.com/sitemap.xml` -> 404, `/store/mt-horeb` -> 404, `/robots.txt` -> 200 but serving Cloudflare's *managed* file (the content-signals block), not the Worker's. `/health` and `/api/*` do reach the Worker, so the custom domain is bound -- only these three path patterns are missing.
    ...
    - **Confirmed 2026-08-02: `worker/wrangler.toml` has no `routes` key at all**, so the entire live route set is dashboard-only. This is why the additive-in-dashboard instruction below matters -- introducing a `routes` key would make wrangler reconcile the live set down to whatever is listed, dropping `/api/*` and friends and taking production down. There is no `wrangler routes list` command; the authoritative set has to come from the dashboard or the REST API.
...
> for the first time since at least 2026-04-08.
...
- [x] **Deploy the Worker** -- done 2026-07-25, version `3e06bb19`. Routes verified on `*.workers.dev`. **But see the routing gap below -- they are still unreachable at the canonical domain.** Original text: -- production 404s on `/sitemap.xml` and `/store/{state}/{city}/{slug}/`. Phase 37 is committed but undeployed, so the "Find Real Users" milestone has no SEO surface live. Blocked on the push above.
- [x] **Verify robots.txt after deploy** -- done 2026-07-25. Confirmed shadowed; folded into the routing item below. Original text: -- custard.chriskaschner.com currently serves Cloudflare's *managed* robots.txt. Phase 37's Worker handler may be shadowed by it. If so, disable the managed robots.txt in the Cloudflare dashboard or the sitemap will not be discoverable.
...
- [ ] **Repair the 22 stale browser specs, then make `browser-tests` blocking** -- Playwright was never actually gating, so specs rotted against shipped refactors. Root causes, in descending size:
  ...
  Note: `.planning/STATE.md` recorded only *one* known browser failure (map-pan-stability); corrected 2026-08-02. Latest CI run reports **21 failed / 11 skipped / 142 passed**.
    - 2026-08-02: added `worker/test/browser/store-picker-mobile.spec.mjs` (4 specs, all passing) -- written during the 2026-04-03 debug session but never committed, so nothing was guarding the iOS auto-focus fix from regressing.
...
- [ ] **Prune stale branches** -- 3 `worktree-agent-*` and 2 `codex/*` branches left over from parallel-agent sessions.
    - Partially done 2026-08-02: the four stale *worktree checkouts* (`.gsd/worktrees/M004`, `.gsd/worktrees/M005`, `.claude/worktrees/agent-a27e30bc`, `.claude/worktrees/agent-a97ea655`) were removed. All branches were kept, so nothing is lost and any can be re-checked-out. This also removes the `.gsd/worktrees/M004` trap where a bare `npx vitest` globbed into its broken `node_modules` and reported phantom failures.
    - Remaining, and needing a judgment call rather than a sweep: `milestone/M005`, `worktree-agent-a27e30bc`, `worktree-agent-a97ea655` are provably fully merged (zero commits not on `main`) and are safe deletes. `milestone/M004` has 7 unique commits, but they are `.gsd/` bookkeeping for the abandoned GSD-2 experiment plus a widget-sync CI job `main` already has under a different name -- so it is effectively superseded, not unmerged work.
    - `codex/security-prod-rollout` (242), `codex/slice-radar-cone-signal-polish` (158), and `worktree-agent-aa6c04fd` (199) have large unmerged sets and have **not** been reviewed. Do not delete these without reading them.
    - Five `~/.codex/worktrees/*` checkouts also point at this repo. They sit outside the repo tree, so they do not affect test collection.
